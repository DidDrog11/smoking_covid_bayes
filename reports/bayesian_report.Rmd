---
title: 'Smoking and COVID Living Review (v12): a bayesian analysis'
...
* **Author dictionary**: An author dictionary is created to manage the presentation of author names in the final tables and meta-analyses. This is particularly for those with punctuation in their names.
* **Load scripts**: We have written functions to handle much of the Bayesian analysis
* **Load Bayesian datasets**: The Bayesian approach relies on large simulated datasets derived from the observed data, for this report we only present the results graphically from data simulated outside of the report. The scripts `bayes_scripts.R`, `bayesian_data_generation.R` and `bayesian_analysis.R` should be able to be run locally to generate your own simulated data.
...
``` {r google_data}
drive_download("https://docs.google.com/spreadsheets/d/1fmTEW0lHYDk1OE3fw9-mJ6xu9EgBPnYoaL05-OTRiOk/edit?usp=sharing", path = here("data_spreadsheet", "data_extraction_current.xlsx"), overwrite = T)
...
flextable(search_details %>%
            mutate(`date_screening in format (2020-05-22)` = ymd(`date_screening in format (2020-05-22)`)))
```
...
datatable(data_study_general[1:4] %>%
            mutate(`date_published_format(2020-05-12)` = ymd(data_study_general$`date_published_format(2020-05-12)`),
...
``` {r excluding_studies}
superseded_studies <- data_study_general %>%
  filter(str_ends(lead_author, "_old")) %>%
  pull(lead_author)

old_isaric <- data_study_general %>%
  filter(str_starts(lead_author, "isaric")) %>%
  head(., -1) %>%
  pull(lead_author)
#This will prevent duplication of the ISARIC reports, only retaining the last based on their names
...
``` {r descriptives}
source(here("scripts", "remove_excluded.R")) # This removes the excluded studies from each table that has been read in previously.
current_total <- nrow(a)
datatable(a)
...
``` {r PRISMA}
previous_total <- read_rds(here("data_clean", "data_study_general.rds")) %>%
...
```
Currently studies are reported from `r paste(nrow(country))` countries.
...
map <- country %>%
  ...
  left_join(., countries %>%
              as_tibble() %>%
              dplyr::select(adm0_a3, geometry), by = c("iso3c" = "adm0_a3")) %>%
  st_as_sf()
...
``` {r interactive global map, cache = TRUE, quietly = TRUE, echo=FALSE, results = "hide"}
from_countries <- c("USA", "UK", "South Korea", "Czechia")
to_countries <- c("United States", "United Kingdom", "Korea", "Czech Rep.")
country_map <- country %>%
...
country_plot <- left_join(country_map  %>%
            rename("name" = Country),
            countries %>%
              select(name),
          by = "name") %>%
...
country_plot$Number[ which(country_plot$Number == 0)] = NA
country_plot$total_cases[ which(is.na(country_plot$total_cases))] = 0


my_bins <- c(0, 1, 5, 10, 20, 50, 70, max(country$Number))
mypalette <- colorBin(palette="YlOrBr", domain = country_plot$Number, na.color="transparent", bins=my_bins)

mytext <- paste(
  "Country: ", country_plot$name, "<br/>", 
  "Number of studies included: ", country_plot$Number, "<br/>", 
  "COVID-19 reported cases (thousands): ", round(country_plot$total_cases, 2), 
  sep="") %>%
  lapply(htmltools::HTML)

html_map <- leaflet(country_plot) %>% 
  addTiles()  %>% 
...
``` {r diagnosis_method}
diagnostics <- table(data_study_general %>%
                       mutate(diagnostic_approach = case_when(is.na(diagnostic_approach) ~ "not_specified",
                                                              TRUE ~ diagnostic_approach)) %>%
                                pull(diagnostic_approach))
diagnostics
```
...
current_smok <- table_1 %>%
  filter(current_smoker != 'NA' | current_vaper != "NA" | current_smoker_vaper != 'NA' | current_smoker_former_vaper != 'NA')
```
...
former_smok <- table_1 %>%
  filter(., former_smoker != 'NA' | former_smoker_vaper != 'NA' | former_smoker_former_vaper != 'NA')
```
* **Former smokers:** `r paste(nrow(former_smok))` studies report on the numbers of former smokers
```{r never smokers}
#Studies reporting never smokers
never_smok <- table_1 %>%
  filter(., never_smoker != 'NA' | never_smoker_vaper != 'NA' | never_smoker_former_vaper != 'NA')
```
...
full_smoking_status <- table_1 %>%
  filter(lead_author %in% current_smok$lead_author & 
           lead_author %in% former_smok$lead_author & 
           lead_author %in% never_smok$lead_author)

b <- full_smoking_status %>%
  select(lead_author, sample_size, current_smoker, former_smoker, never_smoker, current_vaper,
         current_smoker_vaper, former_smoker_vaper, never_smoker_vaper,
         current_smoker_former_vaper, former_smoker_former_vaper, never_smoker_former_vaper,
         study_id) %>%
  rename('Lead author' = lead_author,
         'Sample size' = sample_size,
         'Current smokers' = current_smoker,
         'Current vaper' = current_vaper,
         'Former smokers' = former_smoker,
         'Never smokers' = never_smoker,
         "Current smoker, current vaper" = current_smoker_vaper, 
         "Former smoker, current vaper" = former_smoker_vaper,
         "Never smoker, current vaper" = never_smoker_vaper,
         "Current smoker, former vaper" = current_smoker_former_vaper,
         "Former smoker, former vaper" = former_smoker_former_vaper, 
         "Never smoker, former vaper" = never_smoker_former_vaper,
         'Study ID' = study_id)
...
b <- b %>%
  ...
  colformat_double(j = c("Sample size", "Current smokers", "Former smokers", "Never smokers", "Current vaper",
                         "Current smoker, current vaper", "Former smoker, current vaper", "Never smoker, current vaper",
                         "Current smoker, former vaper", "Former smoker, former vaper", "Never smoker, former vaper"),
...
semi_full_smoking_status <- table_1 %>%
  filter(lead_author %in% current_former_smok$lead_author &
           lead_author %in% never_smok$lead_author &
           !lead_author %in% full_smoking_status$lead_author)
...
country_prevalence_list <- table_1 %>%
  ...
  mutate(current_smok_percentage = current_smoker/total*100,
         former_smok_percentage = former_smoker/total*100,
...
table_1_refs <- table_1 %>%
  left_join(., data_study_general %>%
              select(lead_author, doi),
            by = "lead_author")
#Updating table 1
table_1_word <- table_1 %>%
  mutate(current_percentage = current_smoker/total*100,
         ...
         current_vaper_p = current_vaper/total*100,
         current_smoker_current_vaper_p = current_smoker_vaper/total*100,
         former_smoker_current_vaper_p = former_smoker_vaper/total*100,
         never_smoker_current_vaper_p = never_smoker_vaper/total*100,
         current_smoker_former_vaper_p = current_smoker_former_vaper/total*100,
         former_smoker_former_vaper_p = former_smoker_former_vaper/total*100,
         never_smoker_former_vaper_p = never_smoker_former_vaper/total*100,
         not_stated_percentage = not_stated/total*100,
         missing_percentage = missing/total*100) %>%
  select(lead_author, date_published, country, sample_size, median_age, iqr_lower, iqr_upper, mean_age, lower_range,
         upper_range, standard_deviation, female_sex_percent, 
         current_percentage, current_vaper_p, current_smoker_current_vaper_p, current_smoker_former_vaper_p,
         former_percentage, former_smoker_current_vaper_p, former_smoker_former_vaper_p,
         current_former_percentage,
         never_smoker_percentage, never_smoker_current_vaper_p, never_smoker_former_vaper_p,
         never_smoker_unknown_percentage, not_stated_percentage, missing_percentage, study_id) %>%
...
missingness <- table_6 %>%
  ...
  left_join(., table_1_word %>%
              select(lead_author, missing_percentage, never_smoker_unknown_percentage)) %>%
  group_by(lead_author) %>%
  mutate(missing_percentage = as.numeric(missing_percentage),
         missing = sum(missing_percentage, never_smoker_unknown_percentage, na.rm = T),
         low_missingness = ifelse(missing >= 20, "No", "Yes")) %>%
  distinct(., .keep_all = T)
...
a <- a %>%
  select(study_id, lead_author, date_published, country, sample_size, study_setting, median_mean, mean_used, iqr,
         ...
         current_former_percentage,
         never_smoker_percentage, never_smoker_current_vaper_p, never_smoker_former_vaper_p,
         never_smoker_unknown_percentage, 
         missing_percentage, study_quality_final) %>%
  ...
  rename('Study ID' = study_id,
         ...
         "Current vaper %" = current_vaper_p,
         "Current smoker, current vaper %" = current_smoker_current_vaper_p,
         "Current smoker, former vaper %" = current_smoker_former_vaper_p,
         'Former smoker %' = former_percentage,
         "Former smoker, current vaper %" = former_smoker_current_vaper_p,
         "Former smoker, former vaper %" = former_smoker_former_vaper_p,
         'Current/former smokers %' = current_former_percentage,
         'Never smokers %' = never_smoker_percentage,
         "Never smoker, current vaper %" = never_smoker_current_vaper_p,
         "Never smoker, former vaper %" = never_smoker_former_vaper_p,
...
numeric_columns <- c('Median (IQR)', 'Female %', 'Current smoker %', "Current vaper %", "Current smoker, current vaper %" , "Current smoker, former vaper %",
                     'Former smoker %', "Former smoker, current vaper %", "Former smoker, former vaper %",
                     'Current/former smokers %',
                     'Never smokers %', "Never smoker, current vaper %", "Never smoker, former vaper %",
                     'Never/unknown smokers %', 'Missing %')

saveRDS(a %>%
          select(doi, everything()) %>%
          select(-"Study ID") %>%
          rename("ref" = doi),
        file = here("data_clean", "table_1_refs.rds"))

# The references in table script pulls in the doi information from the manuscript.bib file in reports
source(here("scripts", "references_in_table.R"))
```
The total number of included studies is `r paste(nrow(table_1_word))`. This table is searchable. The references are added in automatically from Zotero. The bibtex file is saved as `manuscript.bib` and is updated following each search.
```{r}
datatable(a) %>%
  formatRound(
    columns = c(8:21),
...
table_2_word <-  table_2 %>%
  mutate(., sample = contributing_sample,
         negative_test_percentage =formatC(negative_test/sample*100, digits = 2, format = "f"),
         ...
         negative_current_vaper_p = formatC(negative_current_vaper/negative_test*100, digits = 2, format = "f"),
         negative_current_smoker_vaper_p = formatC(negative_current_smoker_vaper/negative_test*100, digits = 2, format = "f"),
         ...
         positive_never_former_percentage = formatC(positive_never_former/positive_test*100, digits = 2, format = "f"),
         positive_current_vaper_p = formatC(postitive_current_vaper/positive_test*100, digits = 2, format = "f"),
         positive_current_smoker_vaper_p = formatC(positive_current_smoker_vaper/positive_test*100, digits = 2, format = "f"),
         positive_not_stated_percentage = formatC(positive_not_stated/positive_test*100, digits = 2, format = "f")) %>%
  select(-data_on_testing, -missing, -date_published, -sample, -c(negative_current_former_vaper, negative_former_vaper, positive_current_former_vaper, positive_former_vaper)) %>%
...
a <- table_2_word %>%
  ...
  mutate(Author = lead_author,
         ...
         N_current_vaper = paste(paste(negative_current_vaper,
                                       (negative_current_vaper_p), sep = " (")
                                 , "%)", sep = ""),
         N_current_vaper = na_if(N_current_vaper, "NA ( NA%)"),
         N_current_smoker_vaper = paste(paste(negative_current_smoker_vaper,
                                              (negative_current_smoker_vaper_p), sep = " (")
                                        , "%)", sep = ""),
         N_current_smoker_vaper = na_if(N_current_smoker_vaper, "NA ( NA%)"),
  ...
  mutate(SARS_CoV_2_positive = paste(
    ...
    P_current_vaper = paste(paste(postitive_current_vaper,
                                  (positive_current_vaper_p), sep = " (")
                            , "%)", sep = ""),
    P_current_vaper = na_if(P_current_vaper, "NA ( NA%)"),
    P_current_smoker_vaper = paste(paste(positive_current_smoker_vaper,
                                         (positive_current_smoker_vaper_p), sep = " (")
                                   , "%)", sep = ""),
    P_current_smoker_vaper = na_if(P_current_smoker_vaper, "NA ( NA%)"),
  ...
  select(Author, Population_tested, SARS_CoV_2_negative, N_current_smoker, N_former_smoker, N_current_former_smoker,
         N_never_smoker, N_current_vaper, N_current_smoker_vaper, N_not_stated,
         SARS_CoV_2_positive, P_current_smoker, P_former_smoker, P_current_former_smoker,
         P_never_smoker, P_current_vaper, P_current_smoker_vaper, P_not_stated)
...
a$Author  <- a$Author  %>%
  recode("Bello, Chavolla" = "Bello-Chavolla",
         ...
         "Del, Valle" = "del Valle")
a$Author <- to_upper_camel_case(a$Author, sep_out = ", ")
author_list <- plyr::mapvalues(a$Author,
                               from = cleaned_names,
                               to = correct_names)

a$Author <- author_list

numeric_columns <- c('Population_tested',
                     "SARS_CoV_2_negative", "N_current_smoker", "N_former_smoker", "N_current_former_smoker",
                     "N_never_smoker", "N_current_vaper", "N_current_smoker_vaper", "N_not_stated",
                     "SARS_CoV_2_positive", "P_current_smoker", "P_former_smoker", "P_current_former_smoker",
                     "P_never_smoker", "P_current_vaper", "P_current_smoker_vaper", "P_not_stated")
a <- flextable(a) %>%
...
a <- set_header_labels(a,
                       ...
                       N_current_vaper = "Current vaper (%)",
                       N_current_smoker_vaper = "Current vaper and smoker (%)",
                       ...
                       P_current_vaper = "Current vaper (%)",
                       P_current_smoker_vaper = "Current vaper and smoker (%)",
                       P_not_stated = "Not stated (%)") %>%
  add_header_row(top = TRUE, values = c("","SARS-CoV-2 negative", "SARS-CoV-2 positive" ), colwidths = c(2, 8, 8)) %>%
  theme_booktabs() %>%
  fix_border_issues() %>%
  set_table_properties(width = 1, layout = 'autofit')  %>%
...
included_studies <- quality_table_2 %>%
  filter(overall_quality != 'poor') %>%
  select(lead_author) %>%
  filter(!lead_author %in% c("reiter", "hippisley_cox", "ullah", "lowe", "polat"))

good_quality_2 <- quality_table_2 %>%
  filter(overall_quality == 'good')

fair_quality_2 <- quality_table_2 %>%
  filter(lead_author %in% included_studies$lead_author) %>%
  filter(overall_quality == 'fair')
...
meta <- tibble('author' = table_2$lead_author,
               ...
               'negative_current_vaper' = table_2$negative_current_vaper,
               'positive_current_vaper' = table_2$postitive_current_vaper,
               'negative_current_smoker_vaper' = table_2$negative_current_smoker_vaper,
               'positive_current_smoker_vaper' = table_2$positive_current_smoker_vaper) %>%
...
if(file.exists(here("data_clean", "bayesian_models", "ecdf_list.RDS"))) {
...
table_3_word <- table_3 %>%
  mutate(sample = sample_with_outcome,
         community_percentage = formatC(number_community/sample*100, digits = 2, format = "f"),
         community_current_smoker_percent = formatC(community_current_smoker/number_community*100, digits = 2, format = "f"),
         community_former_smoker_percent = formatC(community_former_smoker/number_community*100, digits = 2, format = "f"),
         community_current_former_smoker_percent = formatC(community_current_former_smoker/number_community*100, digits = 2, format = "f"),
         community_never_smoker_percent = formatC(community_never_smoker/number_community*100, digits = 2, format = "f"),
         community_never_unknown_smoker_percent = formatC(community_never_unknown_smoker/number_community*100, digits = 2, format = "f"),
         community_current_vaper_p = formatC(community_current_vaper/number_community*100, digits = 2, format = "f"),
         community_former_vaper_p = formatC(community_former_vaper/number_community*100, digits = 2, format = "f"),
         community_current_former_vaper_p = formatC(community_current_former_vaper/number_community*100, digits = 2, format = "f"),
         community_not_stated_percent = formatC(community_not_stated/number_community*100, digits = 2, format = "f"),
         number_hospitalised_percent = formatC(number_hospitalised/sample*100, digits = 2, format = "f"),
         hospitalised_current_smoker_percent = formatC(hospitalised_current_smoker/number_hospitalised*100, digits = 2, format = "f"),
         hospitalised_former_smoker_percent = formatC(hospitalised_former_smoker/number_hospitalised*100, digits = 2, format = "f"),
         hospitalised_current_former_smoker_percent = formatC(hospitalised_current_former_smoker/number_hospitalised*100, digits = 2, format = "f"),
         hospitalised_never_smoker_percent = formatC(hospitalised_never_smoker/number_hospitalised*100, digits = 2, format = "f"),
         hospitalised_never_unknown_smoker_percent = formatC(hospitalised_never_unknown_smoker/number_hospitalised*100, digits = 2, format = "f"),
         hospitalised_current_vaper_p = formatC(hospitalised_never_unknown_smoker/number_hospitalised*100, digits = 2, format = "f"),
         hospitalised_former_vaper_p = formatC(hospitalised_former_vaper/number_hospitalised*100, digits = 2, format = "f"),
         hospitalised_current_former_vaper_p = formatC(hospitalised_current_former_vaper/number_hospitalised*100, digits = 2, format = "f"),
         hospitalised_not_stated_percent = formatC(hospitalised_not_stated/number_hospitalised*100, digits = 2, format = "f")) %>%
  select(lead_author, sample_with_outcome, number_community, community_percentage,
         ...
         community_current_vaper, community_current_vaper_p, community_former_vaper, community_former_vaper_p,
         community_current_former_vaper, community_current_former_vaper_p,
         ...
         hospitalised_never_smoker, hospitalised_never_smoker_percent, hospitalised_current_vaper, hospitalised_current_vaper_p, hospitalised_former_vaper, hospitalised_former_vaper_p,
         hospitalised_current_former_vaper, hospitalised_current_former_vaper_p,
         hospitalised_never_unknown_smoker, hospitalised_never_unknown_smoker_percent, hospitalised_not_stated, hospitalised_not_stated_percent) %>%
...
table_4_word <- table_4 %>%
  mutate(sample = sample_with_severity,
         non_severe_disease_percentage = formatC(non_severe_disease/sample*100, digits = 2, format = "f"),
         non_severe_current_smoker_percent = formatC(non_severe_current_smoker/non_severe_disease*100, digits = 2, format = "f"),
         non_severe_former_smoker_percent = formatC(non_severe_former_smoker/non_severe_disease*100, digits = 2, format = "f"),
         non_severe_current_former_smoker_percent = formatC(non_severe_current_former_smoker/non_severe_disease*100, digits = 2, format = "f"),
         non_severe_never_smoker_percent = formatC(non_severe_never_smoker/non_severe_disease*100, digits = 2, format = "f"),
         non_severe_never_unknown_smoker_percent = formatC(non_severe_never_unknown_smoker/non_severe_disease*100, digits = 2, format = "f"),
         non_severe_not_stated_percent = formatC(non_severe_not_stated/non_severe_disease*100, digits = 2, format = "f"),
         severe_disease_number_percent = formatC(severe_disease_number/sample*100, digits = 2, format = "f"),
         severe_disease_current_smoker_percent = formatC(severe_disease_current_smoker/severe_disease_number*100, digits = 2, format = "f"),
         severe_disease_former_smoker_percent = formatC(severe_disease_former_smoker/severe_disease_number*100, digits = 2, format = "f"),
         severe_disease_current_former_smoker_percent = formatC(severe_disease_current_former_smoker/severe_disease_number*100, digits = 2, format = "f"),
         severe_disease_never_smoker_percent = formatC(severe_disease_never_smoker/severe_disease_number*100, digits = 2, format = "f"),
         severe_disease_never_unknown_percent = formatC(severe_disease_never_unknown/severe_disease_number*100, digits = 2, format = "f"),
         severe_disease_not_stated_percent = formatC(severe_disease_not_stated/severe_disease_number*100, digits = 2, format = "f")) %>%
...
table_5_word <- table_5 %>%
  mutate(sample = sample_with_deaths,
         deaths_percentage = formatC(deaths/sample*100, digits = 2, format = "f"),
         death_current_smokers_percent = formatC(death_current_smokers/deaths*100, digits = 2, format = "f"),
         death_former_smokers_percent = formatC(death_former_smokers/deaths*100, digits = 2, format = "f"),
         death_current_former_smokers_percent = formatC(death_current_former_smokers/deaths*100, digits = 2, format = "f"),
         death_never_smokers_percent = formatC(death_never_smokers/deaths*100, digits = 2, format = "f"),
         death_never_unknown_smokers_percent = formatC(death_never_unknown_smokers/deaths*100, digits = 2, format = "f"),
         death_not_stated_percent = formatC(death_not_stated/deaths*100, digits = 2, format = "f"),
         recovered_percentage = formatC(recovered/sample*100, digits = 2, format = "f"),
         recovered_current_smokers_percent = formatC(recovered_current_smoking/recovered*100, digits = 2, format = "f"),
         recovered_former_smokers_percent = formatC(recovered_former_smoker/recovered*100, digits = 2, format = "f"),
         recovered_current_former_smokers_percent = formatC(recovered_current_former_smokers/recovered*100, digits = 2, format = "f"),
         recovered_never_smokers_percent = formatC(recovered_never_smoker/recovered*100, digits = 2, format = "f"),
         recovered_never_unknown_smokers_percent = formatC(recovered_never_unknown_smoker/recovered*100, digits = 2, format = "f"),
         recovered_not_stated_percent = formatC(recovered_not_stated/recovered*100, digits = 2, format = "f")) %>%

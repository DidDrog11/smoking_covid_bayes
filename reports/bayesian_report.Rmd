...
## Download data
``` {r google_data}
sheets_id <- as_sheets_id("https://docs.google.com/spreadsheets/d/1sTlqMbGiWDWJT3dTpa3YQU4z6qcWb0v6Wkp40RcWLXQ/edit?usp=sharing")
...
The pull requests from the API may happen too rapidly if this is run as a chunk so it's better to run this line by line.
...
exclude_from_analysis <- c(superseded_studies, old_isaric, excluded_other_reason)
```
...
my_bins <- c(0, 1, 5, 10, 20, 30, 40, 80)
...
``` {r diagnosis_method}
table(data_study_general$diagnostic_approach)
```

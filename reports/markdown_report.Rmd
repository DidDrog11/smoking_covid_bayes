...
We import the data from the dataset that is available in the "data for analysis" folder on the Google drive. This is available to be viewed with the [link](sheets_id) and is only available for viewing not editing. 
...
flextable(head(search_details[,1:11]))
```
...
This is a searchable list of all the studies that have met out inclusion and exclusion criteria up until `r paste(max(search_details$date_screened))`
...
```
We define the current version of the report here. At the moment this needs to be changed directly when compiling the code. This report has been generated for version `r paste(current_version)`

``` {r excluding_studies}

exclude_from_analysis <- c('isaric_1', 'isaric_2', 'isaric_3', 'miyara_old', 'isaric_4', 'mehra', 'hopkinson', 'gaibazzi', 'miyara_updated', 'russell', "petrilli_old", "magagnoli_old", "niedzwiedz_old", "bello_chavolla_old", "kimmig_old", "russell_old", "zuo_zuo_old", "patel_old")
```
We don't want to remove studies from the dataset despite them no longer being included in the analysis. The excluded studies are defined in this chunk of code. We have excluded this version the following studies: `r knitr::combine_words(exclude_from_analysis)`.
...
The PRISMA reporting guidelines for systematic reviews the guidance is available [here](http://www.prisma-statement.org/). **In Progress**
...
html_map
```
This map displays the number of studies produced in each country. The number of confirmed COVID-19 cases are shown when resting over each country for reference, data on confirmed cases have been obtained from [Rami Krispin's `coronavirus` package](https://ramikrispin.github.io/coronavirus/). The number of cases is updated when the report is generated so will not remain in date, it is current as of `r paste(Sys.time())`
...
![Figure 3: Current versus never smokers](figure/fig_3.png)
...
![Figure 4: Former versus never smokers](figure/fig_4.png)
...
event_rates_smoker
```
![Figure 5: Current versus never smokers](figure/fig_5.png)
...
running_meta_count <- running_meta_count %>%
  ...
  select(study)
```
![Figure 6: Former versus never smokers](figure/fig_6.png)
...
event_rates_smoker
```
![Figure 7: Current versus never smokers](figure/fig_7.png)
...
running_meta_count <- running_meta_count %>%
  ...
  select(study)
```
![Figure 8: Former versus never smokers](figure/fig_8.png)
...
event_rates_smoker
```
![Figure 9: Current versus never smokers](figure/fig_9.png)
...
![Figure 10: Former versus never smokers](figure/fig_10.png)
...
poor <- filter(quality_rating, quality_rating$overall_quality == "poor" ) %>%
  ...
  nrow()
```
The number of **Fair** quality studies are `r paste(fair)` with `r paste(poor)` **Poor** quality studies
